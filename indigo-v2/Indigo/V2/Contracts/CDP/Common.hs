...
module Indigo.V2.Contracts.CDP.Common
  ( CDPScriptParams (..),
    InterestRate (InterestRate, irRate, irActiveFrom),
    IAssetContent
      ( IAssetContent,
        iaName,
        iaMaintenanceRatio,
...
data InterestRate = InterestRate
  { -- | Rate as a fraction, i.e.
    -- `OnChainDecimal {get_on_chain_int: 1_000_000} == 100%`
    -- `OnChainDecimal {get_on_chain_int: 500_000} == 50%`
    irRate :: OnChainDecimal,
    irActiveFrom :: Ledger.POSIXTime
  }
  deriving (P.Show, P.Eq)

PlutusTx.makeLift ''InterestRate
PlutusTx.makeIsDataIndexed ''InterestRate [('InterestRate, 0)]
...
data IAssetContent = IAssetContent
  { iaName :: Value.TokenName,
    ...
    -- | Rates related to CDP interests.
    -- The ordering is from newest to oldest,
    -- i.e. [InterestRate {active_from: 1 day ago,..}, .., InterestRate {active_from: 30 days ago, ..}]
    iaBaseRates :: [InterestRate],
    iaFirstIAsset :: Bool,
...
data AccumulatedFees
  = InterestIAssetAmount
      { iiaLastUpdated :: Ledger.POSIXTime,
        iiaIAssetAmount :: Integer
      }
  | FeesLovelacesAmount
...
PlutusTx.makeLift ''AccumulatedFees
PlutusTx.makeIsDataIndexed ''AccumulatedFees [('InterestIAssetAmount, 0), ('FeesLovelacesAmount, 1)]
...
data CDPContent = CDPContent
  { -- | Nothing means that the CDP is frozen and can be liquidated
    ...
    cdpAccumulatedFees :: AccumulatedFees
  }
  deriving (P.Show)
...
data CDPRedeemer
  = -- | AdjustCDP to modify an existing CDP.
    AdjustCDP {aCurrentTime :: Ledger.POSIXTime}
