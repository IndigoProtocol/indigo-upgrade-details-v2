...
module Indigo.Common.Data.Decimal
...
ceil :: OnChainDecimal -> Integer
ceil a =
  let whole = getOnChainInt a `PlutusPrelude.divide` decimalUnit
   in if getOnChainInt a > whole * decimalUnit
        then whole + 1
        else whole
